import type { GraphNode, LayoutNode, SimpleTreeOptions } from '../types';
...
export function computeSimpleTreeLayout<T>(
  ...
  const widthMap = buildSubtreeWidthMap(nodes, { nodeWidth, siblingGap });
  ...
  const assignPositions = (node: GraphNode<T>, left: number, depth: number, parentId?: string) => {
    const width = widthMap.get(node.id) ?? nodeWidth;
    const children = node.children ?? [];
    const childrenWidth =
      children.length === 0
        ? 0
        : children.reduce((sum, child) => sum + (widthMap.get(child.id) ?? nodeWidth), 0) +
          siblingGap * (children.length - 1);
    const y = depth * (nodeHeight + levelGap);
    let x = left + width / 2;

    if (parentAlignment === 'first-child' && children.length > 0) {
      const firstChildWidth = widthMap.get(children[0].id) ?? nodeWidth;
      const firstChildLeft = left + (width - childrenWidth) / 2;
      x = firstChildLeft + firstChildWidth / 2;
    }
    ...
    for (const child of children) {
      ...
      currentLeft += (widthMap.get(child.id) ?? nodeWidth) + siblingGap;
    }
  };
  ...
  for (const node of nodes) {
    ...
    currentLeft += (widthMap.get(node.id) ?? nodeWidth) + siblingGap;
  }

  const firstRoot = layoutNodes.find(node => node.id === nodes[0].id);
  if (!firstRoot) return layoutNodes;

  // Normalize so the first root sits at x=0 for stable output.
  const shiftX = -firstRoot.x;
  if (shiftX === 0) return layoutNodes;
